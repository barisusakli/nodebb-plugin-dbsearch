...
var pubsub = nbbRequire('src/pubsub');
...
exports.createIndices = function(language, callback) {
	...
	if (nconf.get('isPrimary') === 'true' && !nconf.get('jobsDisabled')) {
		async.series([
			...
			function(next) {
				db.pool.query('CREATE INDEX IF NOT EXISTS "idx__searchtopic__content" ON "searchtopic" USING GIN (to_tsvector($1::regconfig, "content"))', [language], next);
			},
			function(next) {
			...
			function(next) {
				db.pool.query('CREATE INDEX IF NOT EXISTS "idx__searchpost__content" ON "searchpost" USING GIN (to_tsvector($1::regconfig, "content"))', [language], next);
			},
			function(next) {
...
exports.changeIndexLanguge = function(language, callback) {
	...
	async.series([
		...
		function(next) {
			db.pool.query('CREATE INDEX "idx__searchtopic__content" ON "searchtopic" USING GIN (to_tsvector($1::regconfig, "content"))', [language], next);
		},
		function(next) {
			db.pool.query('DROP INDEX "idx__searchpost__content"', next);
		},
		function(next) {
			db.pool.query('CREATE INDEX "idx__searchpost__content" ON "searchpost" USING GIN (to_tsvector($1::regconfig, "content"))', [language], next);
		}
	], function(err) {
		if (err) {
			winston.error(err);
