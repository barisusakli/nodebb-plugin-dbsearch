...
search.init = function(params, callback) {
	...
	db.getObject('nodebb-plugin-dbsearch', function(err, data) {
		...
		searchModule.createIndices(data ? data.indexLanguage || 'en' : 'en');
	});
};
...
function getGlobalAndPluginData(callback) {
	async.parallel({
		...
		var languageSupported = nconf.get('database') === 'mongo';
		var languages = [
			{ name: 'danish', value: 'da', selected: false },
			{ name: 'dutch', value: 'nl', selected: false },
			{ name: 'english', value: 'en', selected: true },
			{ name: 'finnish', value: 'fi', selected: false },
			{ name: 'french', value: 'fr', selected: false },
			{ name: 'german', value: 'de', selected: false },
			{ name: 'hungarian', value: 'hu', selected: false },
			{ name: 'italian', value: 'it', selected: false },
			{ name: 'norwegian', value: 'nb', selected: false },
			{ name: 'portuguese', value: 'pt', selected: false },
			{ name: 'romanian', value: 'ro', selected: false },
			{ name: 'russian', value: 'ru', selected: false },
			{ name: 'spanish', value: 'es', selected: false },
			{ name: 'swedish', value: 'sv', selected: false },
			{ name: 'turkish', value: 'tr', selected: false },
		];
...
socketAdmin.plugins.dbsearch.changeLanguage = function(socket, language, callback) {
	async.waterfall([
		function (next) {
			searchModule.changeIndexLanguage(language, next);
		},
		function (next) {
