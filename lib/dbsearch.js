...
search.filterSearchQuery = async function (data) {
	...
	if (data.hasOwnProperty('content')) {
				query.content = nodeJieba.cut(data.content).join(" ");
	}
...
async function topicsSave(topics) {
	...
	let data = topics.map((topicData) => {
		const indexData = {};
		if (topicData.title) {
			indexData.content = nodeJieba.cut(topicData.title).join(" ");
		}
		if (topicData.cid) {
...
async function postsSave(posts) {
	...
	let data = posts.map((postData) => {
		const indexData = {};
		if (postData.content) {
				indexData.content = nodeJieba.cut(postData.content).join(" ");
		}
		if (postData.cid) {
