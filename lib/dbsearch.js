...
search.init = async function (params) {
	...
	await searchModule.createIndices(convertLanguageName(pluginConfig ? pluginConfig.indexLanguage || 'zh' : 'zh'));
...
search.filterSearchQuery = async function (data) {
	...
	if (data.hasOwnProperty('content')) {
			if (data.content!== undefined ){
					let seg_list = data.content.split(" ").map((x)=>{
							if (testChinese.test(x)){
									return nodeJieba.cut(x).join(" ")
							}
							return x
					})
					query.content = seg_list.join(" ");
			}
	}
...
async function topicsSave(topics) {
	...
	let data = topics.map((topicData) => {
		const indexData = {};
		if (topicData.title) {
				let seg_list = topicData.title.split(" ").map((x)=>{
						if (testChinese.test(x)){
								return nodeJieba.cut(x).join(" ")
						}
						return x
				})
			indexData.content =seg_list.join(" ");
		}
		if (topicData.cid) {
...
async function postsSave(posts) {
	...
	let data = posts.map((postData) => {
		const indexData = {};
		if (postData.content) {
				let seg_list = postData.content.split(" ").map((x)=>{
						if (testChinese.test(x)){
								return nodeJieba.cut(x).join(" ")
						}
						return x
...
async function getPluginData() {
	...
	data.indexLanguage = data.indexLanguage || 'zh';
...
async function getGlobalAndPluginData() {
	...
	plugin.languageSupported = languageSupported;
	plugin.languages = languages;
	plugin.indexLanguage = plugin.indexLanguage || 'zh';
