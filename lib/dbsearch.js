...
search.init = async function (params) {
	...
	pubsub.on('nodebb-plugin-dbsearch-korean:settings:save', (data) => {
...
search.filterMessagingSearchMessages = async function (data) {
	...
	if (data.hasOwnProperty('content') && data.content) {
		query.content = tokenize(data.content);
	}
	if (!Object.keys(query).length) {
		return [];
	}
...
search.reindex = async function () {
	await db.setObject('nodebb-plugin-dbsearch-korean', {
		topicsIndexed: 0,
		postsIndexed: 0,
		messagesIndexed: 0,
		working: 1,
...
const excludeChars = new Set('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!"#$%&\'()*+,-./:;<=>?@[\\]^_`{|}~·￥…（）—【】、；：‘“”’《》，。？\t\n\x0b\x0c\r '.split(''));
...
function processText(text) {
    ...
    try {
        const result = mecab.nounsSync(text);
        const chars = new Set(text);  // 문자 집합을 빠르게 생성
        result.forEach(item => chars.add(item));  // 형태소 분석 결과를 집합에 추가

        // 제외할 문자를 집합에서 제거
        excludeChars.forEach(char => chars.delete(char));

        // 문자 집합을 문자열로 변환
        const resultString = Array.from(chars).join(' ');
        return text.length < 500 ? `${resultString} ${text}` : resultString;
    } catch (err) {
        console.error(err);
        return text;
    }
...
function tokenize(text) {
    // 숫자와 문자 사이에 공백 추가
    const separatedText = text.replace(/(\D)(\d)/g, '$1 $2');
    // 두 가지 텍스트 변환을 수행하고 결과를 결합
    return [text, separatedText].map(processText).join(' ');
}
...
async function topicsSave(topics) {
	...
	await db.incrObjectFieldBy('nodebb-plugin-dbsearch-korean', 'topicsIndexed', result.tids.length);
}
...
async function postsSave(posts) {
	...
	await db.incrObjectFieldBy('nodebb-plugin-dbsearch-korean', 'postsIndexed', result.pids.length);
}
...
async function messagesSave(msgs) {
	...
	let data = msgs.map((msgData) => {
		const indexData = {};
		if (msgData.content) {
			indexData.content = tokenize(msgData.content);
		}
		if (msgData.roomId) {
	...
	await db.incrObjectFieldBy('nodebb-plugin-dbsearch-korean', 'messagesIndexed', result.mids.length);
}
...
async function searchRemove(key, ids) {
	...
	if (key === 'topic') {
		await db.incrObjectFieldBy('nodebb-plugin-dbsearch-korean', 'topicsIndexed', -ids.length);
	} else if (key === 'post') {
		await db.incrObjectFieldBy('nodebb-plugin-dbsearch-korean', 'postsIndexed', -ids.length);
	} else if (key === 'chat') {
		await db.incrObjectFieldBy('nodebb-plugin-dbsearch-korean', 'messagesIndexed', -ids.length);
	}
}
...
async function reIndexPids(pids, topic) {
	if (!Array.isArray(pids) || !pids.length) {
		winston.warn('[nodebb-plugin-dbsearch-korean] invalid-pid, skipping');
		return;
	}
...
async function save(req, res) {
	if (utils.isNumber(req.body.postLimit) && utils.isNumber(req.body.topicLimit)) {
		...
		await db.setObject('nodebb-plugin-dbsearch-korean', data);
		...
		pubsub.publish('nodebb-plugin-dbsearch-korean:settings:save', pluginConfig);
...
async function getPluginData() {
	const data = await db.getObject('nodebb-plugin-dbsearch-korean') || {};
...
socketAdmin.plugins.dbsearch.changeLanguage = async function (socket, language) {
	...
	await db.setObject('nodebb-plugin-dbsearch-korean', { indexLanguage: language });
};
